...
# About the Dataset 
The Datasets used in this analysis come from two sources:
...
# Data Preparation
- source and cleaning:
- For this analysis we will be using two datasets with similar variables.

Dataset 1:

- this data set was sourced from the National UFO Reporting Center (NUFORC), accessed via Kaggle.
- Each row is a reporting sightingv(observation)
- With variables:
  datetime,  
  city, 
  state, 
  ...
  latitude, and 
  lonitude. 
- The dataset cover reports from 1910-2014, predominatly from the United States. 
- Majority of self-reports are submitted online. 
  ```{r}
  
  ```
  
- Useful for patterns over time and location, but because of self-reports theres misclassification and median driven spikes. 
```{r}
glimpse(ufo_data)
```
